...
function newRoom(code, hostId, hostName, now) {
  return {
    ...
    missSeq: 0, // 不一致のたびに+1。クライアントが不正解音を鳴らす。
    judged: false, // オープン後、判定済みか（演出中の二重押し防止）
    history: [],
  };
}
...
// 判定する。ここでは画面遷移せず reveal のまま（回答カードを見せたまま演出）。
// 演出後、ホストが continueRound() で次へ進める。
function judge(room, id, award, celebrate) {
  const h = requireHost(room, id);
  if (!h.ok) return h;
  if (room.phase !== 'reveal') return { ok: false, error: 'オープン後に判定してください' };
  if (room.judged) return { ok: true }; // 二重押し防止（演出中）
  ...
  else room.missSeq = (room.missSeq || 0) + 1; // 不一致の音トリガー
  room.history.push({
    round: room.round,
    topic: room.topic,
...
// 判定の演出が終わったら次へ（ホスト操作）。
function continueRound(room, id) {
  const h = requireHost(room, id);
  if (!h.ok) return h;
  if (room.phase !== 'reveal') return { ok: true };
